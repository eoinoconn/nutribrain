/**
 * Day detail view (T-075, spec §7/§8): `/day/:date`, the dashboard's
 * primary correction surface.
 *
 * Same layout as the Today view (T-074) for an arbitrary date — reuses the
 * shared header/calorie-ring/macro-bars/micro-row pieces from
 * `../components/DaySummary` and the `MealEditor` (T-073) for logging new
 * meals — plus, unlike Today, every logged item is editable and deletable,
 * whole meals are deletable, and there is a manual "set calories out"
 * override input (§8) for days Intervals/Garmin never synced.
 *
...
export default function DayPage(): JSX.Element {
  const { date: routeDate } = useParams<{ date: string }>();
  const localDate = routeDate ?? "";
  ...
  return (
    <section className="space-y-8">
      <DayHeader day={day} title="Day detail" />
