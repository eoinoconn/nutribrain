...
describe("DayPage", () => {
  ...
  it("renders the empty state with day-detail copy when there are no meals", async () => {
  ...
  it("shows and submits the manual calories-out override", async () => {
  ...
  it("deletes a meal item: optimistic removal, API call, success toast", async () => {
    ...
    // First fetch has the item; the post-deletion refetch (triggered by the mutation's
    // onSettled invalidate) reflects the authoritative, item-free day, so the optimistic
    // removal isn't clobbered by a stale re-fetch bringing the item back.
  ...
  it("shows the sync button when the route date is the browser's today", async () => {
    const now = new Date();
